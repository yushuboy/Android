...
Java_org_dp_facedetection_MainActivity_facedetect(JNIEnv *env, jobject /* this */, jlong matAddr) {
    ...
    pResults = facedetect_cnn(pBuffer, (unsigned char *) (bgr.ptr(0)), bgr.cols, bgr.rows,
                              (int) bgr.step);
    ...
    jfieldID faceConfidence = env->GetFieldID(faceClass, "faceConfidence",
                                              "I");//获取int类型参数confidence
    ...
    for (int i = 0; i < (pResults ? *pResults : 0); i++) {
        ...
        __android_log_print(ANDROID_LOG_ERROR, JNITag,
                            "face_rect=[%d, %d, %d, %d], confidence=%d, angle=%d\n", x, y, w, h,
                            confidence, angle);
